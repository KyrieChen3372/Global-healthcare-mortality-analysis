...
## 📖 Overview
This project investigates the intricate relationship between **global healthcare worker density** (physicians, pharmacists, and nurse-midwives) and various **mortality rates** (infant, chronic disease, and suicide). Utilizing a comprehensive UN SDG (Sustainable Development Goals) dataset covering 164 countries from 2000 to 2015, the system quantifies the impact of resource maldistribution on public health outcomes.

> **Core Insight:** Analysis confirms that as health worker density increases, mortality rates decrease significantly. Specifically, in any year where nurse density exceeded 150 per 10,000 population, infant mortality rates approached zero.
...
* **Target:** Categorized countries into **three risk tiers** (Low, Medium, High) based on mortality quantiles.
...
* **Inverse Relationship:** A significant negative correlation exists between health worker density and neonatal/maternal mortality rates across regions.
* **Critical Thresholds:** Analysis shows that areas with almost zero nurses frequently experience infant mortality rates exceeding 40%.
...
* **Suicide Rates:** Male suicide mortality consistently exceeds female rates across all analyzed regions.
* **Regional Peak:** Europe boasts the highest male suicide rate among all continents, followed by Asia.
* **Infant Trends:** Median infant mortality rates are similar for boys and girls (~5%), though regional outliers show extreme localized risks.

### 📍 Regional Trends (2000–2015)
* **Global Improvement:** All regions (Africa, Asia, Europe, Oceania, Americas) showed a decline in child mortality over the study period.
* **The African Outlier:** Africa consistently maintains the highest maternal and child mortality rates coupled with the lowest healthcare worker density.
...
## 🛠️ Tech Stack
* **Data Preprocessing:** `Pandas` (ETL, column renaming, quantile cutting, data normalization).
* **Machine Learning:** `Scikit-Learn` (LogisticRegression, GaussianNB, train_test_split, metrics).
* **Visualization:** `Matplotlib`, `Seaborn` (Violin Plots, Box Plots, Scatter Plots, Bubble Charts).
